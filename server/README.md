...
若不知道**如何在本地 windwos 环境下**启动`Redis`服务，请自行寻找解决方案方案或参看以下内容[如何在 windows 中使用 Docker —— 以 Redis 为例 | kpmark](http://60.205.131.158/blog/2025/02/03/2025-2-3/)

推荐使用 Redis Insight 监控 `Redis`监控缓存内容
...
- **错误响应:**
  ...
  - **状态码:** 500
    - **内容:** `{ "message": "邮件发送失败" }`
- **备注:**
  - 服务器会将验证码发送到用户指定的邮箱，用户需在 5min 内使用该验证码修改密码。
